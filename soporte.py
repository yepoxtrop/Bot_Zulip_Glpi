...
class SoporteHandler():
    ...
    def handle_message(self, message: Dict[str, Any], bot_handler: AbstractBotHandler) -> None:
        ...
        if bot_handler.storage.get(f"{message['sender_full_name']}@{DOMINIO_CORPORATIVO}")["process"] == "/ayuda" or message["full_content"].lower() == "/ayuda":
            
            # Si el comando no esta inicializado
            if bot_handler.storage.get(f"{message['sender_full_name']}@{DOMINIO_CORPORATIVO}")["step"] == None:
            ...
            elif bot_handler.storage.get(f"{message['sender_full_name']}@{DOMINIO_CORPORATIVO}")["step"] == "caso":
            ...
            # Se pregunta al usuario si tiene el id del caso
            elif bot_handler.storage.get(f"{message['sender_full_name']}@{DOMINIO_CORPORATIVO}")["step"] == "id_ticket":
                ...
                if len(list_message) == 2 and list_message[0].lower() == "":
                    ...
                    try:
                        code_ticket = int(list_message[1]);
                    
                        info_ticket = find_ticket(code_ticket);
                    ...
                    bot_handler.send_reply(message, "Comando no válido, recuerda que las opción valida es:\n `/<numero_ticket>` (ejemplo: `/4168`)");           
            
        # Comando /caso -> Comando principal para:
        # - Crear casos - Para versiones posteriores
        # - Ofrecer canales de comunicacion
        elif bot_handler.storage.get(f"{message['sender_full_name']}@{DOMINIO_CORPORATIVO}")["process"] == "/caso" or message["full_content"].lower() == "/caso":
        ...
        # - Verificar el codigo de rustdesk segun el os
        elif bot_handler.storage.get(f"{message['sender_full_name']}@{DOMINIO_CORPORATIVO}")["process"] == "/rustdesk" or message["full_content"].lower() == "/rustdesk":
            ...
            # Se pregunta al usuario si tiene un caso activo
            elif bot_handler.storage.get(f"{message['sender_full_name']}@{DOMINIO_CORPORATIVO}")["step"] == "os":
